...
public class CalibrationPassthroughPoles : MonoBehaviour
{
    ...
    // Art to disable at runtime
    [SerializeField] private List<GameObject> _artToDisable;

    private const float Height = 1.25f;

    public static Transform OvrRootTransform => PlayerOVR.Instance ? PlayerOVR.Instance.transform.parent : null;
    ...
    private void Start()
    {
        foreach (var obj in _artToDisable)
        {
            if (obj) obj.SetActive(false);
        }
        _calibrationOffset.position = Vector3.zero;
        _calibrationOffset.rotation = Quaternion.identity;
        _calibrationController01.position = _virtualPole01.position + Vector3.up * Height;
        _calibrationPole01.position = _virtualPole01.position;
        _calibrationController02.position = _virtualPole02.position + Vector3.up * Height;
    ...
    public void ResetCalibration()
    {
        ...
        _calibrationPole02.position = pos + Vector3.down * Height;
    }
    ...
    public void Calibrate()
    {
        ...
        _calibrationController01.position = _calibrationPole01.position + Vector3.up * Height;
        _calibrationController01.rotation = Quaternion.identity;
        _calibrationController02.position = _calibrationPole02.position + Vector3.up * Height;
        _calibrationController02.rotation = Quaternion.identity;

        if (!OvrRootTransform) return;

        // Adjust player position and rotation based on the player ref
        OvrRootTransform.position += _playerRef.position - MainCameraTransform.position;
        var rotationOffset = _playerRef.rotation * Quaternion.Inverse(MainCameraTransform.rotation);
        ...
        //var yOffset = _playerRef.rotation.eulerAngles.y + (_playerRef.rotation.eulerAngles.y - MainCameraTransform.rotation.eulerAngles.y);
        //OvrRootTransform.rotation = Quaternion.Euler(new Vector3(0.0f, yOffset, 0.0f));
        
        Debug.Log("Position Check" + MainCameraTransform.position + " " + _playerRef.position);
        Debug.Log("Rotation Check" + MainCameraTransform.eulerAngles + " " + _playerRef.eulerAngles);
    }
}
